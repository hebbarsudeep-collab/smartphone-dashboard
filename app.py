...
LEGEND_H = dict(bgcolor="rgba(0,0,0,0)", orientation="h", y=1.12, x=0, font=dict(size=10))
AXIS_DEF  = dict(showgrid=False, zeroline=False, tickfont=dict(color=MUTED))
AXIS_GRID = dict(showgrid=True, gridcolor=BORDER, zeroline=False, tickfont=dict(color=MUTED))


def section_title(title, sub=""):
    children = [html.P(title, style={"color": TEXT, "fontWeight": 700, "fontSize": 13,
                                      "textTransform": "uppercase", "letterSpacing": "0.05em", "margin": 0})]
    if sub:
        children.append(html.P(sub, style={"color": MUTED, "fontSize": 11, "margin": "2px 0 14px 0"}))
...
app.layout = html.Div(
    style={"backgroundColor": BG, "minHeight": "100vh", "fontFamily": "Inter, Segoe UI, sans-serif", "padding": "24px"},
    children=[
        html.Div(
            ...
            children=[
                ...
                html.Div([
                    ...
                    html.Div("June 10, 2026 · 09:00 IST", style={"fontSize": 12, "color": "#94a3b8", "fontWeight": 600}),
                ...
                html.Div(
                    ...
                    children=[
                        html.Div([
                            ...
                            dcc.Checklist(
                                id="filter-brand",
                                options=[{"label": html.Span(b, style={"color": BRAND_COLORS.get(b, TEXT), "marginRight": 6}), "value": b} for b in BRANDS],
                                value=BRANDS, inline=True,
                        ...
                        html.Div([
                            ...
                            dcc.Dropdown(
                                ...
                                value="All", clearable=False, style={"width": 150, "fontSize": 12}
                            ),
                        ]),
                        html.Div([
        ...
        html.Div(id="kpi-row", style={"display": "grid", "gridTemplateColumns": "repeat(4,1fr)", "gap": "16px", "marginBottom": "16px"}),
        ...
        html.P(
            "Smartphone Review Intelligence Dashboard · 50,000 reviews · Jan 2023–Dec 2024 · Internal Use Only",
...
def update_all(brands, country, platform, year, sentiment, verified):
    ...
    def kpi_card(label, value, sub, icon, trend, up, color):
        return html.Div(
            style={**CARD_STYLE, "marginBottom": 0, "position": "relative", "overflow": "hidden"},
            children=[
    ...
    for col, clr in [("Positive", "#34d399"), ("Neutral", "#fbbf24"), ("Negative", "#f87171")]:
        fig_sent_brand.add_trace(go.Bar(name=col, x=bs["brand"], y=bs[col], marker_color=clr, marker_line_width=0))
    fig_sent_brand.update_layout(**BASE_LAYOUT, barmode="stack", showlegend=True, legend=LEGEND_H, xaxis=AXIS_DEF, yaxis=AXIS_GRID)
    ...
    fig_rating = go.Figure(go.Bar(
        ...
        marker_color=["#f87171", "#fb923c", "#fbbf24", "#a3e635", "#34d399"][:len(rc)],
        text=rc["count"].apply(lambda v: f"{v:,}"),
        textposition="outside", textfont=dict(color=MUTED, size=10), marker_line_width=0,
    ))
    fig_rating.update_layout(**BASE_LAYOUT, xaxis=AXIS_DEF,
                              yaxis=dict(tickfont=dict(color="#fbbf24", size=13), showgrid=False, zeroline=False))

    monthly = d.groupby("year_month").agg(total=("rating", "count"), avg_rating=("rating", "mean")).reset_index().sort_values("year_month")
    monthly["label"] = pd.to_datetime(monthly["year_month"]).dt.strftime("%b %y")
    fig_monthly = go.Figure()
    fig_monthly.add_trace(go.Scatter(x=monthly["label"], y=monthly["total"], name="Reviews",
                                      mode="lines", line=dict(color=ACCENT, width=2), yaxis="y1"))
    fig_monthly.add_trace(go.Scatter(x=monthly["label"], y=monthly["avg_rating"], name="Avg Rating",
                                      mode="lines", line=dict(color="#fbbf24", width=2, dash="dot"), yaxis="y2"))
    fig_monthly.update_layout(**BASE_LAYOUT, legend=LEGEND_H, xaxis=AXIS_DEF, yaxis=AXIS_GRID,
                               yaxis2=dict(overlaying="y", side="right", range=[2.5, 3.5],
                                           tickfont=dict(color=MUTED), showgrid=False, zeroline=False))

    ms = d["brand"].value_counts().reset_index()
    ms.columns = ["brand", "count"]
    fig_pie = go.Figure(go.Pie(
        labels=ms["brand"], values=ms["count"], hole=0.5,
        marker=dict(colors=[BRAND_COLORS.get(b, ACCENT) for b in ms["brand"]], line=dict(color=BG, width=2)),
    ...
    for col, clr in [("Positive", "#34d399"), ("Neutral", "#fbbf24"), ("Negative", "#f87171")]:
        fig_country.add_trace(go.Bar(name=col, x=cs["country"], y=cs[col], marker_color=clr, marker_line_width=0))
    fig_country.update_layout(**BASE_LAYOUT, barmode="stack", legend=LEGEND_H, xaxis=AXIS_DEF, yaxis=AXIS_GRID)
    ...
    for _, row in brand_feat.iterrows():
        ...
        fig_radar.add_trace(go.Scatterpolar(
            r=vals, theta=features + [features[0]], name=row["brand"],
            fill="toself", fillcolor=BRAND_COLORS.get(row["brand"], ACCENT),
    ...
    fig_radar.update_layout(**BASE_LAYOUT,
                             polar=dict(bgcolor="rgba(0,0,0,0)", gridshape="linear",
                                        radialaxis=dict(visible=True, range=[2.4, 3.0], gridcolor=BORDER, tickfont=dict(color=MUTED, size=9)),
                                        angularaxis=dict(tickfont=dict(color=TEXT, size=11), gridcolor=BORDER)))

    sc = d.groupby(["brand", "model"]).agg(avg_price=("price_usd", "mean"), avg_rating=("rating", "mean"), count=("rating", "count")).reset_index()
    fig_scatter = go.Figure()
    for brand in sc["brand"].unique():
        sub = sc[sc["brand"] == brand]
        fig_scatter.add_trace(go.Scatter(
            x=sub["avg_price"], y=sub["avg_rating"], mode="markers", name=brand,
            marker=dict(size=sub["count"].apply(lambda v: max(8, min(40, v / 80))),
                        color=BRAND_COLORS.get(brand, ACCENT), opacity=0.8, line=dict(width=0)),
            text=sub["model"],
            hovertemplate="<b>%{text}</b><br>Price: $%{x:.0f}<br>Rating: %{y:.2f}<extra></extra>",
        ))
    fig_scatter.update_layout(**BASE_LAYOUT,
                               xaxis=dict(title="Price (USD)", showgrid=True, gridcolor=BORDER, zeroline=False, tickfont=dict(color=MUTED), tickprefix="$"),
                               yaxis=dict(title="Avg Rating", showgrid=True, gridcolor=BORDER, zeroline=False, tickfont=dict(color=MUTED)))
    ...
    fig_plat.update_layout(**BASE_LAYOUT, xaxis=AXIS_DEF,
                            yaxis=dict(tickfont=dict(color=TEXT, size=11), showgrid=False, zeroline=False))
    ...
    for col, clr in [("Positive", "#34d399"), ("Neutral", "#fbbf24"), ("Negative", "#f87171")]:
        fig_verified.add_trace(go.Bar(name=col, x=vd["label"], y=vd[col], marker_color=clr, marker_line_width=0))
    fig_verified.update_layout(**BASE_LAYOUT, barmode="stack", legend=LEGEND_H, xaxis=AXIS_DEF, yaxis=AXIS_GRID)
    ...
    fig_area = go.Figure()
    fig_area.add_trace(go.Scatter(x=st["label"], y=st["Positive"], name="Positive", stackgroup="one",
                                   mode="lines", line=dict(color="#34d399", width=1.5), fillcolor="rgba(52,211,153,0.25)"))
    fig_area.add_trace(go.Scatter(x=st["label"], y=st["Neutral"], name="Neutral", stackgroup="one",
                                   mode="lines", line=dict(color="#fbbf24", width=1.5), fillcolor="rgba(251,191,36,0.25)"))
    fig_area.add_trace(go.Scatter(x=st["label"], y=st["Negative"], name="Negative", stackgroup="one",
                                   mode="lines", line=dict(color="#f87171", width=1.5), fillcolor="rgba(248,113,113,0.25)"))
    ...
    table = dash_table.DataTable(
        ...
        style_header={"backgroundColor": BG, "color": MUTED, "fontWeight": 700,
                       "fontSize": 10, "textTransform": "uppercase", "border": f"1px solid {BORDER}"},
        style_cell={"backgroundColor": CARD_BG, "color": TEXT, "fontSize": 12,
                     "padding": "10px 12px", "border": f"1px solid {BORDER}"},

...
# Base layout — NO legend, xaxis, yaxis (added per chart as needed)
BASE_LAYOUT = dict(
...
def update_all(brands, country, platform, year, sentiment, verified):
    ...
    for col, color in [("Positive", "#34d399"), ("Neutral", "#fbbf24"), ("Negative", "#f87171")]:
        fig_sent_brand.add_trace(go.Bar(name=col, x=bs["brand"], y=bs[col],
                                         marker_color=color, marker_line_width=0))
    fig_sent_brand.update_layout(**BASE_LAYOUT, barmode="stack", showlegend=True,
                                  legend=LEGEND_H,
                                  xaxis=AXIS_DEF, yaxis=AXIS_GRID)
    ...
    fig_rating.update_layout(**BASE_LAYOUT,
                              xaxis=AXIS_DEF,
    ...
    for col, color in [("Positive", "#34d399"), ("Neutral", "#fbbf24"), ("Negative", "#f87171")]:
        fig_country.add_trace(go.Bar(name=col, x=cs["country"], y=cs[col],
                                      marker_color=color, marker_line_width=0))
    fig_country.update_layout(**BASE_LAYOUT, barmode="stack",
                               legend=LEGEND_H, xaxis=AXIS_DEF, yaxis=AXIS_GRID)
    ...
    fig_scatter.update_layout(**BASE_LAYOUT,
                               xaxis=dict(title="Price (USD)", showgrid=True, gridcolor=BORDER,
                                          zeroline=False, tickfont=dict(color=MUTED), tickprefix="$"),
    ...
    fig_plat.update_layout(**BASE_LAYOUT,
                            xaxis=AXIS_DEF,
    ...
    for col, color in [("Positive", "#34d399"), ("Neutral", "#fbbf24"), ("Negative", "#f87171")]:
        fig_verified.add_trace(go.Bar(name=col, x=vd["label"], y=vd[col],
                                       marker_color=color, marker_line_width=0))
    fig_verified.update_layout(**BASE_LAYOUT, barmode="stack",
                                legend=LEGEND_H, xaxis=AXIS_DEF, yaxis=AXIS_GRID)
